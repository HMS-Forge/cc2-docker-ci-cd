...
> **Примечание:** Данный репозиторий является **форком** проекта [train-tracker] из предыдущей работы по Docker. Часть README по CI/CD находится после примеров запросов.
>
> В рамках практической работы по CI/CD в него добавлены:
> - unit‑ и интеграционные тесты (JUnit 5 + H2);
> - линтер (Checkstyle);
> - CI/CD пайплайн (GitHub Actions) со стадиями сборки, линта, тестов (с проверкой покрытия ≥50%), сборки Docker-образа и публикации в Docker Hub.
## Описание проекта
...
В проекте настроен линтер **Checkstyle** для контроля стиля кода. Конфигурационный файл `checkstyle.xml` находится в директории `app/`. Проверить код на соответствие стилю можно командой:
...
Для модульного и интеграционного тестирования используются **JUnit 5**, **Mockito** и **H2** (in‑memory база данных). Тесты охватывают:

- Сервисный слой (юнит-тесты с Mockito)
- Репозитории (интеграционные тесты с H2)
- Контроллеры (MockMvc)
- Мапперы MapStruct
- DTO и валидацию
...
После выполнения будет сгенерирован отчёт покрытия в `app/target/site/jacoco/index.html`. Порог покрытия установлен на уровне **50%** – если покрытие ниже, сборка упадёт.

### CI/CD Pipeline (GitHub Actions)

При открытии Pull Request или пуше в ветки `main`/`develop` автоматически запускается пайплайн, который выполняет:

| Job | Описание |
|-----|----------|
| `build` | Компиляция проекта через Maven (`mvn clean compile`) |
| `lint` | Проверка стиля кода (Checkstyle) |
| `test` | Запуск всех тестов и проверка покрытия (JaCoCo, порог 50%). Также публикуется отчёт покрытия как артефакт сборки. |
| `docker-build` | Сборка Docker-образа на основе Dockerfile из `app/`. Тег образа формируется из хеша коммита (`:${{ github.sha }}`). |
| `docker-push` | (Только при пуше в `main`/`develop`) Публикация образа в Docker Hub. Теги: `:commit-sha` и `:latest`. |

Все стадии обязательны – при любой ошибке (неудачная сборка, нарушение линтера, падение тестов или покрытие <50%) пайплайн останавливается и помечается как failed.

**Секреты GitHub:** для публикации в Docker Hub используются переменные `DOCKER_USERNAME` и `DOCKER_PASSWORD`, зашифрованные в настройках репозитория.
...
2. Перейдите в папку `app`:
   ...
   mvn clean verify          
   ```
3. Откройте `target/site/jacoco/index.html` для просмотра покрытия.

Линтер можно запустить отдельно:
```bash
mvn checkstyle:check
```

Все тесты используют встроенную H2, поэтому запускать Docker для тестирования не требуется.
